...
git init 创建版本库
件和本地件，但是在提交之前，还是可以通过git reset HEAD && git checkout —- <file>恢复件
...
git push origin :refs/tags/<tagname>可以删除一个远程标签。git add 添加件到缓冲区
...
git checkout -- <file> revert件到add过或最新的版本，先add的版本，再最新的版本
